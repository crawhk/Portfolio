...
```{r}
Thesis <- read_spss("C:/Users/crawl/OneDrive/Documents/GitHub/Portfolio/ThesisAllDataNewNew.sav")
```
...
```{r}
  cor.test(Thesis$RPLieAvg, Thesis$RP_CVxOCE, method = "pearson")
```

```{r}
ggplot(Thesis, aes(x = RPLieAvg, y = RP_CVxOCE)) +
  geom_jitter(alpha = 0.3) + 
  geom_smooth(method = "lm", se = TRUE) +  
  labs(
    x = "RP Lying Average (RPLieAvg)",
    y = "RP Outcome Value (RP_CVxOCE)",
...
```{r}
  cor.test(Thesis$TDLieAvg, Thesis$TD_CVxOCE, method = "pearson")
```

```{r}
ggplot(Thesis, aes(x = TDLieAvg, y = TD_CVxOCE)) +
  geom_jitter(alpha = 0.3) + 
  geom_smooth(method = "lm", se = TRUE) +  
  labs(
    x = "TD Lying Average (TDLieAvg)",
    y = "TD Outcome Value (TD_CVxOCE)",
